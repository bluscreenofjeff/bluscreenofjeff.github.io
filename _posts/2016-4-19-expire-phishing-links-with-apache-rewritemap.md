...
<span style="color: gold">Change the entire request</span> <span style="color: orange"> to '/<span style="color: dodgerblue">original request path</span>?id=' and append the value returned by process.py.</span> <span style="color: tomato"> Redirect the user, changing the address bar, but continue evaluating rules.</span><br>
